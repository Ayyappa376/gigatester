...
(function($) {
    ...
    var Feedback = {
        ...
        // static_url: "https://static.gigatester.io",
        widget_css: "./gigatester.css",
        scripts: {
            html2canvas: "https://static.gigatester.io/widget/slocal.min.js",
            rrweb: "https://static.gigatester.io/widget/record-1.0.7.min.js"
        },
        http_header: [],
        canvas_mode: false,
        canvas_target: false,
        ...
        form_settings_default: {
                ...
                submitAnnotation: function() {
                    ...
                    if (Feedback.Tools.annotation_count === 0) {
                        if (Feedback.ui.overlay) {
                            Feedback.ui.overlay.addClass("_gigatester-blank")
                        }
                    }
                    ...
                    Feedback.Tools.removeTools();
                    Feedback.hideComments();
                    if (!GigaTester.screen_capture && !Feedback.configs.is_live) {
                        Feedback.setScreenStatus("Taking screenshot...")
                    }
                        ...
                        Feedback.capture(function(data_uri) {
                            ...
                            if (Feedback.canvas_mode) {
                                Feedback.screenshot_data_uri.push({
                                    ...
                                    window_y: document.body.scrollHeight,
                                    window_top: 0,
                                    window_left: 0
                                })
                            } else {
                    ...
                    }, GigaTester.screen_capture ? 20 : 0)
                },
            ...
            addScreenshotBadge: function() {
                if (this.video_url) {
                    this.ui.controls.find(".gigatester-controls-video-preview").show();
                    this.ui.controls.find(".gigatester-controls-screenshot, .gigatester-controls-video").attr("disabled", "disabled");
                    this.ui.controls.find(".gigatester-controls-video .gigatester-screenshot-preview-checkmark").show();
                    this.ui.controls.find(".gigatester-controls-screenshot-preview").hide()
                } else {
                    this.ui.controls.find(".gigatester-controls-screenshot-preview").show();
                    this.ui.controls.find(".gigatester-controls-screenshot, .gigatester-controls-video").attr("disabled", "disabled");
                    this.ui.controls.find(".gigatester-controls-screenshot .gigatester-screenshot-preview-checkmark").show();
                    this.ui.controls.find(".gigatester-controls-video-preview").hide();
                    this.ui.controls.find(".gigatester-controls-screenshot-counter span").text(this.screenshot_data_uri.length);
                    if (GigaTester.screen_capture) {
                        this.ui.controls.find(".gigatester-screenshot-preview img").remove();
                        $(this.screenshot_data_uri).each(function(index, screenshot_data) {
                            var preview_img = $("<img>").addClass("screenshot-preview").attr("src", screenshot_data.data_uri);
                            this.ui.controls.find(".gigatester-screenshot-preview").append(preview_img)
                        }.bind(this))
                    }
                }
            ...
            recordImage: async function(e){
                ...
                if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
                    ...
                    recorder.onstop = e => {
                        console.log(recorder.state)
                        this.ui.element.attr("drawing", "false");
                        ...
                        setTimeout(()=> (this.screenshotVideo()), 700);
            ...
            screenshotVideo: function(){
                ...
                Feedback.form_data.comment_field =  Feedback.form_data.rating
            ...
            post: function(){
                ...
                const postData = {
                    ...
                    feedbackMedia: {
                      ...
                      productKey: 'BxiX1h1RQE9WyP9ewAJhu7RhHVOGkhC469ZrqrD7',
                ...
                var _post_success = function(post_data) {
                    ...
                    switch (form_settings.outro_icon) {
                        ...
                        case 5:
                            ...
                            break;
                        case 10:
                            outro_icon = '<gtdiv class="emoji-icon">ðŸŽ‰</gtdiv>';
                            break
                    }
                    var success_icon = $('<gtdiv class="gigatester-controls-send-success">').html("<style>" + ":root {" + "--widget-outro-icon: " + form_settings.outro_icon_colour + " !important;" + "}" + "</style>" + '<gtdiv data-icon="' + form_settings.outro_icon + '">' + outro_icon + "<gtspan>" + Lib.htmlEntities(form_settings.outro_headline) + "</gtspan>" + "<p>" + Lib.htmlEntitiesWithA(form_settings.outro_paragraph, true) + "</p>" + "</gtdiv>" + (this.configs.display_powered_by ? "<gtfooter>" + "<span>Powered by</span>" + "<span>" + " Gigatester" + "</span>"  + "</gtfooter>" : ""));
                    this.ui.controls.append(success_icon);
            ...
            capture: function(callback) {
                if (GigaTester.screen_capture) {
                    ...
                    if (typeof html2canvas === "undefined") {
                        this.loadScripts([this.scripts.html2canvas], function() {
            ...
            addLocalStyles: function() {
                ...
                for (var i = 0; i < document.styleSheets.length; i++) {
                    ...
                    if (style_sheet.ownerNode.tagName === "LINK" && style_sheet.ownerNode.getAttribute("href")) {
                        ...
                        if (style_sheet.href) {
                            style_sheet.ownerNode.setAttribute("gthref", style_sheet.href)
                        }
                    } else if (style_sheet.ownerNode.tagName === "LINK" && style_sheet.ownerNode.integrity) {
                        add_style = true
                    } else if (style_sheet.ownerNode.tagName === "STYLE" && style_sheet.ownerNode.innerText.trim() === "") {
                        add_style = true
                    }
                    if (add_style) {
                        if (style_sheet.cssRules && style_sheet.cssRules.length) {
                            ...
                            if (styles) {
                                $('<style class="gt_local_styles">').html(styles).insertAfter(style_sheet.ownerNode)
                            }
                        }
                    }
                }
            ...
            html2imageCapture: function(callback) {
                ...
                $("body *").add("svg").each(function() {
                    ...
                    var messure_height = element.hasClass("gigatester-lazy-load");
                    if (scroll_top > 0) {
                    ...
                    if (messure_height || tag_name === "VIDEO" || tag_name === "PICTURE" || tag_name === "SELECT" || tag_name === "SVG") {
                        if (element.css("box-sizing") === "border-box") {
                            element.attr("data-gtheight", element.outerHeight())
                        } else {
                            element.attr("data-gtheight", element.height())
                        }
                ...
                $("[data-gtheight]").removeAttr("data-gtheight");
                $("style.gt_local_styles").remove();
                ...
                for (var i = 0; i < canvas_array.length; i++) {
                    ...
                    if ($(canvas_array[i]).is(":visible")) {
                        try {
                            canvas_data_url = canvas_array[i].toDataURL("image/png", 1)
                        } catch (e) {
                            canvas_data_url = ""
                        }
                    } else {
                ...
                document_html = document_html.replace(/<\/body>.*?<\/html>$/i, canvas_data + video_data + iframe_data + "</body></html>");
                document_html = document_html.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, "");
                callback(document_html)
            },
            html2canvasCapture: function(callback) {
                var svg_image_onload = function(svg_img_load_success) {
                    if (this.Tools.snap) {
                        ...
                        html2canvas(document.body, {
                            ...
                            var imageOnload = function() {
                                ...
                                try {
                                    ...
                                    if (error_count > 0) {
                                        this.html2imageCapture($.proxy(callback, this));
                                        return
                                    }
                                    error_count++;
                                    ...
                                    $(canvg_canvas).remove();
                                    svg_img = new Image;
                                    svg_img.onload = $.proxy(function(e) {
                                        imageOnload.call(this, true)
                                    }, this);
                                    svg_img.onerror = $.proxy(function(e) {
                                ...
                                image.onload = function() {
                                    ...
                                    // console.log(svg_img_load_success, 'svg_img_load_success')
                                ...
                                image.src = image_data_uri
                                console.log(image_data_uri, 'img data')
                            };
                            ...
                            console.log(html2canvas_image.src, "canvas img")
                ...
                if (this.Tools.snap) {
                    ...
                    console.log(svg_string, 'svg_string')
                    var svg_annotation_img = new Image 
                    // $("<img>").attr('src', "https://images.unsplash.com/photo-1638913974071-ad0045d13691?ixlib=rb-1.2.1&ixid=MnwxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1974&q=80")
                    console.log(svg_annotation_img, 'annoted img')
                    var url = "data:image/svg+xml;base64," + btoa(svg_string);
                    svg_annotation_img.onload = $.proxy(function(e) {
                        svg_image_onload.call(this, true)
                    }, this);
                    svg_annotation_img.onerror = $.proxy(function(e, b, c, d) {
                        svg_image_onload.call(this, false)
                    }, this);
                    svg_annotation_img.src = url
                    console.log(url, 'annot url')
                } else {
                    svg_image_onload.call(this, false)
                }
            },          
    ...
    var Video_Capture = {
        ...
        events: {
            ...
            documentMouseDown: function(e) {
                ...
                if ($(e.target).parents("gtvideotoolbar").length || $(e.target).prop("tagName").toLowerCase() === "ubvideotoolbar") {
                    return
                }
                ...
                var _afterGetVideoStream = function() {
                    ...
                    var _timer = function() {
                        if (count_down === 0) {
                            ...
                            $("<utimer>").addClass("gigatester-video-count-down").text(count_down).appendTo($(document.body));
                            count_down--;
    ...
    var Comment = function(options) {
        ...
        this.is_new = true;
        var element = $("<div>").addClass("gigatester-comment").attr("data-html2canvas-ignore", "true");
        var comment_message = "";
        ...
        this.add = function() {
            ...
            element.find(".gigatester-button-input").on("click", function(e) {
