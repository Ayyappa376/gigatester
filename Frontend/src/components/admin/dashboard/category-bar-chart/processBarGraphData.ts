...
export const processBarGraphData = (responseData: IResponseData, teamName: string) => {
...
//      const hoverBorderColorVal = `rgba(${rgbColors[i].red.toString()}, ${rgbColors[i].green.toString()}, ${rgbColors[i].blue.toString()},1)`
      ...
      const str = `${el.name}(${el.lowerLimit.toString()}-${el.upperLimit.toString()})`;
//      userLevelStrings.push(str);
      return {
        label: str,
      ...
      Object.keys(responseData.teams).forEach((team: string) => {
        if (teamName === 'all' || teamName === team) {
          responseData.teams[team].assessments.forEach((assessment: any) => {
            if (assessment && assessment.result && assessment.result.categoryWiseResults &&
              assessment.result.categoryWiseResults[el]) {
              markLevel(assessment.result.categoryWiseResults[el].percentage, index);
            }
          })
        }
      })
