...
export const processResultsTableData = (responseData: IResponseData, teamName: string) => {
  ...
  try {
    ...
    questionIds.forEach(el => {
      ...
      Object.keys(questionsDetails[el].answers).forEach(a => {
        weightage[questionsDetails[el].answers[a].weightageFactor * weightageCoefficient] = 0;
      })
      ...
      if (teamName === 'all' || teamName === team) {
        responseData.teams[team].assessments.forEach((assessment: any) => {
          if (assessment && assessment.assessmentDetails) {
            Object.keys(assessment.assessmentDetails).forEach((questId: string) => {
              const questionId = getQuestionIdFromCompositeQuestionId(questId);
...
//              const answerSelected = assessment.assessmentDetails[questId].answers[0]; // here we will use questId as we are accessing the assessment details array.
              const answersSelected = assessment.assessmentDetails[questId].answers; // here we will use questId as we are accessing the assessment details array.
              const numberOfAnswers = answersSelected.length;
              if (answersSelected[0] !== '@N/A') {
                if (returnData[questionId] && returnData[questionId].weightage &&
                  questionsDetails[questionId] && questionsDetails[questionId].answers &&
                  questionsDetails[questionId].answers[answersSelected[0]] &&
                  questionsDetails[questionId].answers[answersSelected[0]].weightageFactor &&
                  typeof returnData[questionId].weightage[(questionsDetails[questionId].answers[answersSelected[0]].weightageFactor * weightageCoefficient).toString()] !== 'undefined' /*&&
                  questionsDetails[questionId].version === questionVersion*/) {
                  // tslint:disable-next-line: no-increment-decrement
                  answersSelected.forEach((element: string) => {
                    // tslint:disable-next-line: no-increment-decrement
                    returnData[questionId].weightage[(questionsDetails[questionId].answers[element].weightageFactor * weightageCoefficient).toString()] += 1;
                  });
//                  returnData[questionId].sortFactor = calculateSortFactor(returnData[questionId], numberOfAnswers - 1)
    ...
    Object.keys(returnData).forEach((qId: string) => {
      ...
      if(totalQs > 0) {
        returnData[qId].sortFactor = Math.round((totalVal / (totalQs * parseInt(maxWeight, 10))) * 100);
      }
    })
  } catch (err) {
