...
(function() {
    ...
    console.log('GigaTester: [Info] dependencies loading');
function gigatester() {
    if(typeof window.jQuery === "undefined" || typeof window.html2canvas === "undefined"  || typeof window.platform === "undefined" ||  typeof window.Snap === "undefined"){
        console.log('GigaTester: [Error] dependencies not loaded');
    }
    else {
        try {
            ...
            (function($) {
                "use strict";
                console.log('GigaTester: [Info] inside main function');
                ...
                for(let i = 0; i < sc.length; i+=1)
                {
                    ...
                    if(s.src && s.src.match(/gigatester_script\.js$/)) {
                        gtSrcUrl = s.src.substring(0, s.src.indexOf('gigatester_script'));
                    }
                }
                if(gtSrcUrl === '') {
                    console.log("GigaTester: [Error] could not locate the source URL.")
                }
                const Lang = {
                    language: "en",
                    en: {
                ...
                let GigaTester_modal = {
                    ...
                    static_css: gtSrcUrl + "gigatester.css",
                    canvas_mode: false,
                    canvas_target: false,
                    controls_step: 0,
                    click_counter: 0,
                    ...
                    addFeedbackButton: function() {
                        ...
                        $(window).on("beforeunload", function(e) {
                            console.log('GigaTester : [Info] Changes not saved')
                            ...
                            for(let i=0; i<sessionStorage.length;i++){
                                ...
                                if(key === 'gigatesterDefaultUserDetails'){
                                    ...
                                    Object.entries(userDetails).forEach(([key, val]) => {
                                        ...
                                        console.log('GigaTester : [Info] userDetails ', key, val);
                                    });
                                }
                            }
                        }
...
//                      routing_items.help = this.configs.help_link ? '<gtroutemenu class="gigatester-feedback-type-help">' + Svg_Icons.feedback_help + "<gtdiv>" + (this.configs.help_title ? String_Validator.validate(this.configs.help_title) : Lang.get("contact_us")) + "</gtdiv>" + "<gtdiv>" + (this.configs.help_message ? String_Validator.validate(this.configs.help_message) : Lang.get("contact_us_help")) + "</gtdiv>" + "</gtroutemenu>" : "";
                    ...
                    recordImage: async function(e){
                        ...
                        if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
                            console.log("GigaTester: [Warn] This browser does not support the API yet");
                            callback();
                        } else {
                            ...
                            navigator.mediaDevices.getDisplayMedia({
                                ...
                                GigaTester_modal.Draw_Tools.removeTools()
                                stream.onended = () => {
                                    console.info("GigaTester: [Info] Recording has ended");
                                };

                                stream.onerror = () => {
                                    console.log('GigaTester: [Error] Error occuring during stream');
                                }
                                const recorder = new MediaRecorder(stream);
                                const chunks = [];
                                recorder.ondataavailable = e => {
                                    chunks.push(e.data);
                                    console.log('GigaTester: [Info] image recorder state ',recorder.state)
                                }
                                if(stream){
                                    recorder.start();
                                    ...
                                    if (videoTracks && videoTracks[0]) {
                                        ...
                                        try {
                                            ...
                                            console.warn('GigaTester: [Warn] ImageCapture not supported on this browser');
                                            recorder.onstop = e => {
                                ...
                                console.log('GigaTester: [Info] image recording started')
                            })
                            .catch(function(err) {
                                console.log(err , 'err')
...
function checkgigatester() {
    ...
    setTimeout(() => {
        checkgigatester();
        console.log('GigaTester: waiting for dependencies to load')}, 200);
    }
    else{
        console.log('GigaTester: dependencies loaded')
        gigatester();
    }
}
checkgigatester()
