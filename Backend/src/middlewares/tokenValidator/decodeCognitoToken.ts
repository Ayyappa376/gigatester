...
*/
    ...
    request({
        ...
        if (error || response.statusCode !== 200) {
            appLogger.error({err: error}, 'Network Error! Unable to download JWTs.');
            return callback({ message: 'network error' }, undefined);
        }

        const pems: object = {};
        appLogger.info({cognito_response_body: body}, 'body received from cognito');
...
//            appLogger.info('keyId=%s, modulus=%s, exponent=%s, keyType=%s', keyId, modulus, exponent, keyType);
        ...
        jwt.verify(token, pem, function (err: any, payload: any): void {
            ...
            if(payload.email.match(/[A-Z]+/)) {
                updateCognitoUserToLowerCase(payload.sub, payload.email).then((res) => {
                    appLogger.info({updateCognitoUserToLowerCase_result: res});
                }).catch((e: any) => appLogger.warn({updateCognitoUserToLowerCase_error: e}));
            }
