...
export function tokenValidator(request: Request, response: Response, next: NextFunction): void {
    //skip the validator if 'noauthvalidate' header is set
    if((request.headers.noauthvalidate) && (request.headers.noauthvalidate === 'true')) {
        appLogger.info({noauthvalidate: request.headers.noauthvalidate}, 'Skipping Token Validation');
        return next();
    }
