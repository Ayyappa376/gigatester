...
export const initializeLoggerAndConfig = async (request: Request, response: Response, next: NextFunction) => {
    ...
    .then((res) => {
        const configDetails: SystemConfigDetails = <SystemConfigDetails>res.config;
    ...
    .catch ((err) => {
        appLogger.error(err, 'Failed to get System Config from database for orgId=%s', orgId);
        next(err);
    });
};
