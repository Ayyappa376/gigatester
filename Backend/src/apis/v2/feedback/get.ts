...
import { appLogger, AssessmentDocument, getAssessmentHistory, responseBuilder } from '@utils/index';
...
async function handler(request: FeedbackRequest, response: Response): Promise<any> {
    ...
    try {
        const assessmentHistory: AssessmentDocument[] = await getAssessmentHistory({ userId, type: 'feedback' });
