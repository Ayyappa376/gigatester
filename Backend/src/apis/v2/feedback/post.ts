...
import { appLogger, responseBuilder, updateAssessmentFeedback } from '@utils/index';
...
async function handler(request: FeedbackRequest, response: Response) {
    ...
    try {
        await updateAssessmentFeedback({ userId: email, assessmentId, details: { rating, comment } });
