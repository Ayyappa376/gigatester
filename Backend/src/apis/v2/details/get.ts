...
import {
    appLogger, AssessmentDetails, AssessmentDocument, getQuestionDetailsMulti,
    getUserAssessment, getUserAssessmentFromIndex, responseBuilder
} from '@utils/index';
...
async function handler(request: DetailsRequest, response: Response): Promise<any> {
    ...
    try {
        const assessmentDocument: AssessmentDocument = isResult === 'true' ? await getUserAssessment({ userId: email, assessmentId }) : await getUserAssessmentFromIndex({ assessmentId });
        appLogger.info({getUserAssessment_getUserAssessmentFromIndex: assessmentDocument});
        ...
        const questionDetailsArr: Question[] = await getQuestionDetailsMulti({ questionIds });
        appLogger.info({getQuestionDetailsMulti: questionDetailsArr});
        const acknowledgement: DetailsAcknowledgement = await getResponseBody({ questionDetailsArr, assessmentDocument });
