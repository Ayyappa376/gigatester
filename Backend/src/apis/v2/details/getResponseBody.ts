...
import { getBestScoringAssessment, getQuestionCategoryFromQuestionnaire, getQuestionnaireId, getRandomize, getResultLevels, sortCategoriesMapByCategories } from '@root/utils/dynamoDb';
...
export async function getResponseBody({ questionDetailsArr, assessmentDocument, userLevels }: { assessmentDocument: AssessmentDocument; questionDetailsArr: Question[]; userLevels?: any }): Promise<DetailsAcknowledgement> {
    const resp = (await getQuestionnaireId(assessmentDocument.type, assessmentDocument.questionnaireVersion));
    const questionnaireRecommendationFlag: boolean = resp.showRecommendations ? resp.showRecommendations : false;
    ...
    const acknowledgement: DetailsAcknowledgement = {
        ...
        hideResult: assessmentDocument.questionnaireDetails ? assessmentDocument.questionnaireDetails.hideResult ? assessmentDocument.questionnaireDetails.hideResult : false : false,
        result: assessmentDocument.result,
        showRecommendations: questionnaireRecommendationFlag,
        timeOut: assessmentDocument.questionnaireDetails ? assessmentDocument.questionnaireDetails.timeOut ? assessmentDocument.questionnaireDetails.timeOut : false : false,
    };
...
*/
    for (const questionId of quesOrder) {
        ...
        const category = await getQuestionCategoryFromQuestionnaire(questionId, assessmentDocument.type, assessmentDocument.questionnaireVersion);
        const randomize = await getRandomize(assessmentDocument.type, assessmentDocument.questionnaireVersion ? assessmentDocument.questionnaireVersion : '1');
        const questionDetails: Question = questionDetailsArr.filter((o) => o.id === questionId)[0];
        ...
        Object.keys(questionDetails.answers).forEach((_answerId) => {
            options.push({
                ...
                weightage: questionDetails.answers[_answerId].weightageFactor * weightageCoefficient
            });
        });
    ...
    acknowledgement.userLevels = userLevels ? userLevels : await getResultLevels();
    const bestScoringAssessment = await getBestScoringAssessment(assessmentDocument.type);
