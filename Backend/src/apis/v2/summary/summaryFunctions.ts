...
export const getIndexesForAssessmentDetails = async (assessmentDetails: AssessmentDetails, questionnaireType: string, questionnaireVersion: string) => {
    const questionnaireQuestionIds = await getQuestionsListSorted({quesType: questionnaireType, version: questionnaireVersion });
    ...
    Object.keys(assessmentDetails).forEach((key: string) => {
        ...
        if(questionnaireQuestionIds.indexOf(questionId) >= 0) {
            ...
            assessmentDetailsWithIndex[key].index = questionnaireQuestionIds.indexOf(questionId);
        }
    });
