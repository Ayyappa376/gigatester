...
import { getQuestionnaire, getQuestionnaireId, getQuestionnairesAssigned, getUserDocument } from '@root/utils/index';
...
async function handler(request: GetQuestionnaire, response: Response) {
    ...
    try {
        if(request.query.questionnaireId && request.query.questionnaireId !== '') {
            const questionnaire = await getQuestionnaireId(request.query.questionnaireId, request.query.questionnaireVersion);
        ...
        const fetchQuestionnaireList = (status === 'all') ? await getQuestionnaire(true) : await getQuestionnaire(false);
            ...
            if (latest) {
                ...
                getQuestionnaireList.forEach((el: any, i: number) => {
                    if (el.questionnaireId === questionnaire.questionnaireId) {
                        itemExist = true;
                        if (parseInt(getQuestionnaireList[i].version, 10) < parseInt(questionnaire.version ? questionnaire.version : '1', 10)) {
        ...
        const teams = (await getUserDocument({ cognitoUserId: user['cognito:username'] })).teams[0];
        appLogger.info({getUserDocument_first_teams: teams});
        const getList: string[] = await getQuestionnairesAssigned(teams ? teams.name : 'Others');
