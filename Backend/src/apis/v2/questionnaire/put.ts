...
import { responseBuilder, appLogger, getTheLatestQuestionnaireVersion, getQuestionnaireId, publishQuestion } from '@utils/index';
import { Response } from 'express';
import { QuestionnaireUpdate, updateQuestionnaire } from '@root/utils/dynamoDb/questionnaireManagement';
...
async function handler(request: PostQuestionnaire, response: Response) {
    ...
    try {
        if(body.type === 'update' && body.questionnaire) {
            ...
            const latestVersion = await getTheLatestQuestionnaireVersion(create.questionnaireId);
            const latestQuestionnaire = await getQuestionnaireId(create.questionnaireId, latestVersion);
            let doVersionUpdate = false;
            
            if (create.questions && create.active) {
                if (latestQuestionnaire.timeOut !== create.timeOut || latestQuestionnaire.hideResult !== create.hideResult ) {
                ...
                if (latestQuestionnaire.questions.length !== create.questions.length) {
                    ...
                    create.questions.forEach ((questionId: string, i:number) => {
                        if (questionId !==  latestQuestionnaire.questions[i] && create.categoriesMap[questionId] !== latestQuestionnaire.categoriesMap[questionId]) {
                           doVersionUpdate = true;
                        }
                    })
                }
            ...
            const create: QuestionnaireUpdate = body.questionnaire;
            if (body.questionnaire && body.questionnaire.questions && body.questionnaire.questions.length === 0) {
                const err = new Error('Please map the questions to the questionnaire before publishing.');
                appLogger.error(err, 'Please map the questions to the questionnaire before publishing..');
            ...
            if(create.active && create.questions) {
                create.questions.forEach((questionId: string) => publishQuestion(questionId) );
            }
