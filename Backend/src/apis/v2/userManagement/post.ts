...
import { addCognitoUser, addDynamoUser, addUserToCognitoGroup, appLogger, getUserDocument, responseBuilder } from '@utils/index';
...
async function handler(request: AddUser, response: Response) {
    ...
    const managerDetails: UserDocument = await getUserDocument({cognitoUserId: headers.user['cognito:username']});
    appLogger.info({getUserDocument: managerDetails});
    if(managerDetails && managerDetails.roles && !managerDetails.roles.includes('Manager') && !managerDetails.roles.includes('Admin')) {
        const err = new Error('InvalidUser');
        appLogger.error(err, 'Forbidden');
        return responseBuilder.forbidden(err, response);
    }
    const userDetails: any = await addCognitoUser(body.emailId, body.teams).catch((e) => {
    ...
    body.roles.forEach((role: string) => {
        addUserToCognitoGroup(userDetails.User.Username, role).then((res: any) => {
            //res
    ...
    const addResponse: any = await addDynamoUser(userDetails.User.Username, managerDetails, body).catch((e) => {
