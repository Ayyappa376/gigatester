...
import { addUserToCognitoGroup, appLogger, getUserDocument, removeUserFromCognitoGroup, responseBuilder, updateDynamoUser } from '@utils/index';
...
async function handler(request: UpdateUser, response: Response) {
    ...
    if(!(headers.user['cognito:groups'][0] === 'Admin') && !userDetails.order.includes(headers.user.email)) {
        const err = new Error('Forbidden Access, Unauthorized user');
        appLogger.error(err, 'Forbidden');
        return responseBuilder.forbidden(err, response);
    }
    ...
    if(userDetails.roles) {
        ...
        for (const role of removeRoles) {
            removeUserFromCognitoGroup(userDetails.id, role).then((res: any) => {
                //result
    ...
    for (const role of body.roles) {
        addUserToCognitoGroup(userDetails.id, role).then((res: any) => {
            //result
