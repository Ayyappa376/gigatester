...
import { appLogger, getCreateUserConfig, getTeamMembersDetails, getTeams2, getUserDocument, getUserDocumentFromEmail, responseBuilder, TeamMembers } from '@utils/index';
...
//    }
    if (params.type === 'createuser') {
        const createUser: any =  await getCreateUserConfig(headers.user.orgId ? headers.user.orgId : config.defaults.orgId, headers.user['cognito:groups'][0] === 'Admin' ? 'admin' : headers.user.email);
    ...
    if(params.type === 'getusers' && query.email) {
        ...
        getUserDetails[0].teams = getUserDetails[0].teams.map((teamName: any) => teamName.name);
        const createUser: any =  await getCreateUserConfig(headers.user.orgId ? headers.user.orgId : config.defaults.orgId, headers.user['cognito:groups'][0] === 'Admin' ? 'admin' : headers.user.email);
        appLogger.info({getCreateUserConfig: createUser});
        return responseBuilder.ok({ config: createUser.config, values: getUserDetails[0]}, response);
    }
    if(params.type === 'allUsers') {
        const teamList: string[] = (await getTeams2(headers.user['cognito:groups'][0] === 'Admin' ? 'admin' : headers.user.email)).map((val: any) => val.teamId);
        appLogger.info({getTeams2: teamList});
        const userDoc: UserDocument = await getUserDocument({ cognitoUserId: headers.user['cognito:username'] });
        ...
        return responseBuilder.ok({ users: teamMembersForATeam, userCount }, response);
    }
    if(params.type) {
        ...
        return responseBuilder.ok({ users: teamMembersDetails, userCount }, response);
    }
