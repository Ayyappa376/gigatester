...
import { appLogger, AssessmentDetails, CategoryWiseResults, getQuestionnaireId, Result } from '@utils/index';

function calculateIncrForSelection(answers: { [id: string]: Answer }, answerSelected: string[], weightageCoefficient: number, isMultiSelect?: boolean, numberOfAnswers?: number) {
    appLogger.info({calculateIncrForSelection: {answers, answerSelected, weightageCoefficient, isMultiSelect, numberOfAnswers}});
    let score = 0;
    let maxScore = 0;

    if((answerSelected.length === 0) || (answerSelected.length > 0 && answerSelected[0] !== '@N/A')) {
...
function calculatePercentageForCategory(score: number, maxScore: number): number {
    return (Math.round(score / maxScore * 100) || 0);
}

function calculateResultPerCategory(category: string, incr: any, categoryWiseResults: CategoryWiseResults): void {
    ...
    if (categoryWiseResults[category]) {
        ...
        categoryWiseResults[category].percentage = calculatePercentageForCategory(categoryWiseResults[category].score,
...
export const calculateResult = async (date: number, userAssessmentDetails: AssessmentDetails, questionDetails: Question[], assessmentId: string, assessmentType: string, questionnaireVersion?: string): Promise<Result> => {
    appLogger.info({calculateResult: {date, userAssessmentDetails, questionDetails, assessmentId, assessmentType, questionnaireVersion}});
    const result: Result = {
        categoryWiseResults: {},
        level: 'NA',
        maxScore: 0,
    ...
    const questionnaireDetails = await getQuestionnaireId(assessmentType, questionnaireVersion);
    ...
    for (const question of questionDetails) {
        ...
        const answerSelected = userAssessmentDetails[qId] ? userAssessmentDetails[qId].answers ?
                                userAssessmentDetails[qId].answers : [] : [];
        const incr = calculateIncrForSelection(question.answers, answerSelected, weightageCoefficient, type === 'multi-select', numberOfAnswers);
    ...
    result.recommendations = (questionnaireDetails.timeOut || questionnaireDetails.hideResult) ? {} : await calculateAnswerQuality(questionDetails, categoryWiseResults, userAssessmentDetails, recommendations, questionnaireDetails);
