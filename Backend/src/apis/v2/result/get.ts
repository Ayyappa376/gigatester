...
import { addResultToAssessmentDocument, appLogger, AssessmentDetails, AssessmentDocument, getQuestionDetailsMulti,
         getQuestionIds, getResult, getResultLevels, responseBuilder, Result } from '@utils/index';
import { Response } from 'express';
import { DetailsAcknowledgement, getResponseBody } from '../details/getResponseBody';
...
async function handler(request: AssessmentResultRequest, response: Response) {
    ...
    try {
        const assessmentDocument: AssessmentDocument = await getResult({ userId, assessmentId });
        ...
        const userAssessmentDetails: AssessmentDetails | undefined = assessmentDocument? assessmentDocument.assessmentDetails : undefined;
        appLogger.info({getUserAssessmentDetails: userAssessmentDetails});
        if (!userAssessmentDetails) {
            const err = new Error(`Assessment: ${assessmentId} does not exist for ${userId}`);
            appLogger.error(err);
            throw err;
        }
        ...
        const questionDetailsAnsweredQuestions: Question[] = questionIdsAnswered.length > 0 ? await getQuestionDetailsMulti({ questionIds: questionIdsAnswered }) : [];
        const questionIdsAll: string[] = await getQuestionIds(type, questionnaireVersion);
        ...
        for (const questionId of questionIdsAll) {
            for (const qId of questionIdsAnswered) {
                if (qId.startsWith(questionId)) {
                    ...
                    questionIdsUnanswered.splice(questionIdsUnanswered.indexOf(questionId), 1);
                }
            }
        }
        ...
        const questionDetailsUnansweredQuestions: Question[] = questionIdsUnanswered.length > 0 ? await getQuestionDetailsMulti({ questionIds: questionIdsUnanswered }) : [];
        const questionDetails: Question[] = [...questionDetailsAnsweredQuestions, ...questionDetailsUnansweredQuestions];
        appLogger.info({questionDetails});
        const userLevels = await getResultLevels();
        appLogger.info({getResultConfig: userLevels});
        const result: Result = await calculateResult(date, userAssessmentDetails, questionDetails, assessmentId, type, questionnaireVersion);
        ...
        const acknowledgement: DetailsAcknowledgement = await getResponseBody({ questionDetailsArr: questionDetails, assessmentDocument });
        appLogger.info({getResponseBody: acknowledgement});
        if(acknowledgement.result) {
           await addResultToAssessmentDocument({ result: acknowledgement.result, userId, assessmentId, dateSubmit: assessmentDocument.dateSubmit });
...
---------------------*/
export async function correctResult(assessmentDocument: AssessmentDocument): Promise<Result> {
    const userAssessmentDetails: AssessmentDetails | undefined = assessmentDocument? assessmentDocument.assessmentDetails : undefined;
    if (!userAssessmentDetails) {
    ...
    const { assessmentId, date, type, questionnaireVersion, userId } = assessmentDocument;
    const questionIdsAnswered: string[] = Object.keys(userAssessmentDetails);
    appLogger.info({questionIdsAnswered});
    const questionDetailsAnsweredQuestions: Question[] = questionIdsAnswered.length > 0 ? await getQuestionDetailsMulti({ questionIds: questionIdsAnswered }) : [];
    const questionIdsAll: string[] = await getQuestionIds(type, questionnaireVersion);
    ...
    appLogger.info({result});
    assessmentDocument.result = result;
    const dateSubmit = assessmentDocument.dateSubmit ? assessmentDocument.dateSubmit : new Date().getTime();
    await addResultToAssessmentDocument({ result, userId, assessmentId, dateSubmit });
    return result;
}
