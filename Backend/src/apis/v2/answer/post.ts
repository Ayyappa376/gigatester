...
import { appLogger, responseBuilder, updateAssessmentDetails } from '@utils/index';
...
async function handler(request: AnswerRequest, response: Response) {
    ...
    try {
        await updateAssessmentDetails({ userId: email, assessmentId, details: { questionId, version, answers, comment } });
