...
import { CollectorAttributesDetails, CollectorConfig, CollectorConfigDetails, FieldConfigAttributes, GeneralConfigDetails, SystemConfigDetails, TeamConfigDetails, UserConfigDetails } from '@models/index';
import { config } from '@root/config';
import { generate } from '@utils/common';
import { appLogger, responseBuilder, setCollectorConfig, setGeneralConfig, setTeamConfig, setUserConfig } from '@utils/index';
import { Response } from 'express';

const NEW_ATTR_KEY_PREFIX = 'newAttr';

interface PostSettings {
    body: SystemConfigDetails | UserConfigDetails | TeamConfigDetails | GeneralConfigDetails | CollectorConfigDetails;
    headers: {
        user: {
...
async function handler(request: PostSettings, response: Response) {
    ...
    switch(params.type) {
        case 'UserConfig': {
            ...
            await setUserConfig(headers.user.orgId ? headers.user.orgId : config.defaults.orgId, configDetails);
            break;
        }
        case 'TeamConfig': {
            const configDetails: TeamConfigDetails = convertNewKeys(body);
            await setTeamConfig(headers.user.orgId ? headers.user.orgId : config.defaults.orgId, configDetails);
            break;
        }
        case 'GeneralConfig': {
            const configDetails: GeneralConfigDetails = <GeneralConfigDetails>body;
            await setGeneralConfig(headers.user.orgId ? headers.user.orgId : config.defaults.orgId, configDetails);
            break;
        }
        case 'CollectorConfig': {
            const configDetails: CollectorConfigDetails = <CollectorConfigDetails>body;
            ...
            await setCollectorConfig(headers.user.orgId ? headers.user.orgId : config.defaults.orgId, configDetails);
            break;
        }
        default: {
...
function convertNewKeys(fieldMap: any) {
    const configDetails: (UserConfigDetails | TeamConfigDetails | CollectorAttributesDetails) = {};
